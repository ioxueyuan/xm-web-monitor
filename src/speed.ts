...
export const observe = (
	...
	try {
		...
		if (
			...
			const po = new PerformanceObserver((l: PerformanceObserverEntryList) =>
				callback(l.getEntries())
			);
...
const getReady = () => {
	try {
		return modifyFloor(
			getNavigationEntryFromPerformanceTiming().domInteractive
		);
	} catch (err) {
...
const getPerformanceEntryList = () => {
	try {
		if (window.performance && window.performance.getEntries) {
			const EntryList: Array<EntryItemInterface> =
